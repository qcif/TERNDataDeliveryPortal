...
if( $registryObject )
{
	...
	if(isContributorPage($registryObjectKey)) {
		...
		$rdaLink = '<br /><a style="font-size:0.8em; font-weight: normal;" href="http://'.$host.'/'.$rda_root . '/view/group/?group='.urlencode($registryObjectKey). '&groupName='.esc($objectGroup).'">'.$rdaLinkPrefix.' this record in TERN Data Discovery Portal</a>'."\n";
	} else {
		$rdaLink = '<br /><a style="font-size:0.8em; font-weight: normal;" href="http://'.$host.'/'.$rda_root . '/' . $url_slug.'">'.$rdaLinkPrefix.' this record in TERN Data Discovery Portal</a>'."\n";
	}
